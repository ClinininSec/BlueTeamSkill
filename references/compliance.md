...
> 本文件把 SKILL.md 的操作红线与默认工作流规范合并、细化为七条编号红线（编号为本文件内部编号），并展开脱敏规则；所有模式（monitor / audit / ir / traffic / remote）通用。
...
以下为本文件内部编号（红线 1-7）；SKILL.md 未对红线编号——其 frontmatter 概述为"不出 PoC / 不做破坏动作 / 不做横移"三条硬红线，正文另列五条操作红线 + 四条默认工作流规范，本文件将其合并展开为下述七条。每条说明：**为什么 / 违反后影响 / 例外情形**。
