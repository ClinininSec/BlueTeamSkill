...
> v0.4-M0 修订：红线 4 从"完全禁止 SSH 远连"改为"四要素约束下允许"；新增红线 7（白名单强制 + Tier 3 默认关）。
...
- **为什么**：v0.1 定位「乙方驻场 + 离线作业」，对客户主网无写入授权；接 API 会触发审计、可能被视为擅自联网，违反驻场规约。
- **影响**：轻则被甲方安全运维 escalate；重则被认为越权访问数据，触发安全合规事件。
- **例外**：v0.4+ 计划支持，需用户在客户脱敏审批流程提交申请且全程留痕。当前版本：严格禁止。
...
### 红线 4：不擅自远连；远连必须四要素达标（v0.4-M0 修订）

- **为什么**：Skill 之前定位为「离线副驾驶」，v0.4-M0 起支持 remote 模式（SSH 远程分析）；但远连是甲方合规敏感动作，必须四要素齐全才能启动。
- **四要素**：
...
### 红线 7：未授权命令不入白名单，Tier 3 默认关（v0.4-M0 新增）
...
- **例外**：无例外。Tier 3 需要新增 cmd_id 时，走"提 PR → e2e 冒烟 → 双人 review → 释放到 v0.4-M2+"的正规扩展流程（见 `references/remote-command-whitelist.md` §七）
