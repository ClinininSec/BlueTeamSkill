...
| Very long qname (>= 60 chars) | R-TRAF-006 (v0.2) | SIG-TRAF-063 |
...
| `ttl` | int | 响应记录的 TTL |
| `answer_len` | int | 响应记录整体长度（v0.3-M1 建议加） |

若客户 pcap 已用 v0.2 版本 `pcap_parser.py`，只有 `qname/qtype/rcode/response_ip` 4 个字段。v0.3-M1 建议**在下次 pcap 采集时** enable `payload_len / ttl / answer_len`（需修改 `pcap_parser.py` view_dns，加入相应 tshark 字段）。
