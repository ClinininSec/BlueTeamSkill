...
**调用**：
```bash
python scripts/nginx_anomaly.py \
...
**调用**：
```bash
python scripts/auth_log_audit.py \
...
**处理**：用通用 `ioc_match.py` + 关键 EventID 过滤即可，无独立脚本（v0.1）。
...
1. **入口脚本**：`python scripts/vendor_field_mapper.py --input <厂商导出> --vendor <name> --output /tmp/hvv-audit-vendor.jsonl`
...
| SSRF 探测 | URL 含 `http://169.254.169.254` / `metadata` | 低 | playbook（v0.2 列入） |
...
| C2 心跳 | 同 dst 周期性外联（间隔 30s/60s/300s） | 中 | playbook（v0.2） |
...
1. **范围对齐**（4 问）
2. **解析 + 字段标准化**（`scripts/log_parser.py`；厂商告警走 `scripts/vendor_field_mapper.py --vendor <name>`）
3. **专用脚本检测**（按日志类型分流到 nginx_anomaly / auth_log_audit / ioc_match）
4. **关联分析**（IP / UA / 账号 / 时间线，可选调用 `agents/log-analyzer` 子 agent 处理大批量）
5. **脱敏**
6. **渲染异常清单 + IOC 列表**

子 agent 介入条件：
- 单日志行数 > 100k 或 多源关联需要复杂语义判断 → 调用 `agents/log-analyzer`
- 否则主会话直跑脚本即可
...
- 厂商字段抽屉（v0.3-M1）：`../log-fields/vendor-{qax-ngsoc,sangfor-sip,changting-safeline,dbappsec-mingyu}.md`
