...
> **🔍 检查点 A（审核）**：本步完成后**必跑** `agents/checkpoint-reviewer`。审核命令清单安全性/Tier 越权 + stdout 异常信号。审核通过进下一步。
...
> **🧭 检查点 B（决策）**：主会话基于审核结论决策下一步（转 ir 取证 / 转 audit 深挖 / 收尾）。
...
- 已经通过 monitor / audit / traffic / ir 定性为真实入侵（P0 / P1，false_positive_prob ≤ 0.1）
...
> **✅ 检查点 C（验证）**：Tier 3 处置后**必跑** `agents/verdict-validator` 验证处置是否达到预期（跑只读 T1 命令复核 + 证据闭环）。
...
> **✅ 检查点 C（验证）**：出终报前**必跑** `agents/verdict-validator` 验证 verdict 证据闭环 + 待跟进 / 处置列表无漏标；rejected 打回检查点 B 重做。（§7.4 的 Tier 3 处置后复核是 remote 专属的额外 C 触点，与此处收尾验证并存。）
...
- 命令白名单知识库：`../remote-command-whitelist.md`
- 白名单命名空间（`R-REM-*` / `R-REM-DISP-*` / `SESSION-AUDIT-*` / `CHECK-LIN-<AREA>-NN`）：`../rule-id-namespaces.md`
- 审计字段：`../log-fields/audit-session.md`
- 分级标准：`../grading.md`
- 脱敏与红线（含远连四要素）：`../compliance.md`
- 统一终报：`../../assets/final-report.md`（remote 形态）+ `../../assets/findings-schema.md`
- 远程脚本：`../../scripts/remote/{ssh_probe,remote_collect,session_recorder}.*`
